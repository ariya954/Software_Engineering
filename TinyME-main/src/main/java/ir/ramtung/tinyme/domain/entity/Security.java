...
public class Security {
    ...
    public MatchResult newOrder(EnterOrderRq enterOrderRq, Broker broker, Shareholder shareholder, Matcher matcher) {
        ...
        if (enterOrderRq.getPeakSize() == 0) {
            if (enterOrderRq.getMinimumExecutionQuantity() == 0) {
                ...
                if(matchingState != MatchingState.AUCTION && enterOrderRq.getStopPrice() > 0 && enterOrderRq.getSide() == Side.BUY)
        ...
        MatchResult matchResult = matcher.execute(order, enterOrderRq.getMinimumExecutionQuantity(), matchingState, openingPrice);
    ...
    public MatchResult updateOrder(EnterOrderRq updateOrderRq, Matcher matcher) throws InvalidRequestException {
        ...
        if(matchingState != MatchingState.AUCTION) {
            matchResult = matcher.execute(order, 0, matchingState, openingPrice);
            if(!matchResult.trades().isEmpty())
                lastTradedPrice = matchResult.trades().getLast().getPrice();
            ...
            tradableQuantity = orderBook.calculateTradableQuantityAccordingTo(openingPrice);
            if (updateOrderRq.getSide() == Side.BUY) {
                originalOrder.getBroker().decreaseCreditBy(order.getValue());
            }
        }
        return matchResult;
    }

    public List<Trade> reopening(Matcher matcher) throws InvalidRequestException {
        ...
        for(Order buyOrder : orderBook.findTradableBuyOrdersAccordingTo(openingPrice)) {
            if (orderBook.findTradableSellOrdersAccordingTo(openingPrice).size() == 0)
                break;
            MatchResult matchResult = matcher.execute(buyOrder, 0, matchingState, openingPrice);
            for (Trade trade : matchResult.trades())
                trades.add(trade);
        }
        lastTradedPrice = openingPrice;
        return trades;
    }
}
